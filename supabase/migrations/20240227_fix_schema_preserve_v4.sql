-- Temporarily rename existing tables to preserve data
alter table if exists nba_schedule rename to nba_schedule_old;
alter table if exists game_odds rename to game_odds_old;
alter table if exists predictions rename to predictions_old;
alter table if exists team_stats rename to team_stats_old;

-- Create team_stats table with correct schema
...
begin
    ...
    if exists (select from pg_tables where schemaname = 'public' and tablename = 'nba_schedule_old') then
        insert into nba_schedule (game_id, game_date, home_team, away_team, team_id_home, team_id_away)
        select game_id, game_date, home_team, away_team, team_id_home, team_id_away
        from nba_schedule_old;
    end if;

    -- Transfer odds data
    if exists (select from pg_tables where schemaname = 'public' and tablename = 'game_odds_old') then
        insert into game_odds (game_id, game_date, spread_home, total, home_moneyline, away_moneyline)
        select game_id, game_date, spread_home, total, home_moneyline, away_moneyline
        from game_odds_old;
    end if;

    -- Transfer predictions data
    if exists (select from pg_tables where schemaname = 'public' and tablename = 'predictions_old') then
        insert into predictions (
            game_id, game_date, home_team, away_team, team_id_home, team_id_away,
            predicted_home_score, predicted_away_score, predicted_total,
            fanduel_spread_home, fanduel_total, home_moneyline, away_moneyline,
            actual_home_score, actual_away_score, spread_result, total_result, game_status
        )
        select 
            game_id, game_date, home_team, away_team, team_id_home, team_id_away,
            predicted_home_score, predicted_away_score, predicted_total,
...
-- Drop old tables after successful transfer
drop table if exists nba_schedule_old;
drop table if exists game_odds_old;
drop table if exists predictions_old;
drop table if exists team_stats_old;